...
Theatre.core = (() => {
  ...
  var getPointerMeta = (p2) => {
    const meta = p2[pointerMetaSymbol];
    return meta;
  };
  var getPointerParts = (p2) => {
    const { root: root2, path } = getPointerMeta(p2);
  ...
  function isPrism(d2) {
    return !!(d2 && d2.isPrism && d2.isPrism === true);
  }
  ...
  var HotHandle = class {
    constructor(_fn, _prismInstance) {
      ...
      for (const d2 of this._dependencies) {
        d2._addDependent(this._reactToDependencyGoingStale);
      }
      startIgnoringDependencies();
      this.getValue();
      stopIgnoringDependencies();
    ...
    removeDependent(d2) {
      this._dependents.delete(d2);
    }
    addDependent(d2) {
      this._dependents.add(d2);
    }
    destroy() {
      for (const d2 of this._dependencies) {
        d2._removeDependent(this._reactToDependencyGoingStale);
      }
     ...
     */
    _addDependency(d2) {
      if (this._dependencies.has(d2))
        return;
      this._dependencies.add(d2);
      d2._addDependent(this._reactToDependencyGoingStale);
    }
    /**
     * @internal
     */
    _removeDependency(d2) {
      if (!this._dependencies.has(d2))
        return;
      this._dependencies.delete(d2);
      d2._removeDependent(this._reactToDependencyGoingStale);
    }
  };
  var emptyObject = {};
  var PrismInstance = class {
    ...
    onStale(callback) {
      const untap = () => {
        this._removeDependent(fn2);
      };
      const fn2 = () => callback();
      this._addDependent(fn2);
      return untap;
    }
    /**
    ...
    _addDependent(d2) {
      ...
      this._state.handle.addDependent(d2);
    }
    _goHot() {
  ...
  var HotScope = class _HotScope {
    ...
    memo(key, fn2, deps) {
      ...
      if (depsHaveChanged(memo2.deps, deps)) {
        startIgnoringDependencies();
        memo2.cachedValue = safelyRun(fn2, void 0).value;
        stopIgnoringDependencies();
        memo2.deps = deps;
      }
  ...
  function safelyRun(fn2, returnValueInCaseOfError) {
    try {
      return { value: fn2(), ok: true };
    } catch (error) {
  ...
  function memo(key, fn2, deps) {
    const scope2 = hookScopeStack.peek();
    if (!scope2) {
      throw new Error("prism.memo() is called outside of a prism() call.");
    }
    return scope2.memo(key, fn2, deps);
  }
  ...
  function scope(key, fn2) {
    ...
    const ret = safelyRun(fn2, void 0).value;
    hookScopeStack.pop();
    return ret;
  }
  function sub(key, fn2, deps) {
    return memo(key, () => prism(fn2), deps).getValue();
  }
  function inPrism() {
  ...
  function calculateColdPrism(fn2) {
  ...
  var getKeyOfValue = (v2, key, vType = getTypeOfValue(v2)) => {
  ...
  var Atom = class {
     ...
     * ```
     */
    setByPointer(pointerOrFn, val3) {
      this.reduceByPointer(pointerOrFn, () => val3);
    }
  ...
  function isPointerToPrismProvider(val3) {
    return typeof val3 === "object" && val3 !== null && val3["$$isPointerToPrismProvider"] === true;
  }
  ...
  var Ticker = class {
     ...
     */
    onThisOrNextTick(fn2) {
      this._scheduledForThisOrNextTick.add(fn2);
      if (this._dormant) {
        this._goActive();
      }
    }
     ...
     */
    onNextTick(fn2) {
      this._scheduledForNextTick.add(fn2);
      if (this._dormant) {
        this._goActive();
      }
    }
     ...
     * @see onThisOrNextTick
     */
    offThisOrNextTick(fn2) {
      this._scheduledForThisOrNextTick.delete(fn2);
    }
    /**
     ...
     */
    offNextTick(fn2) {
      this._scheduledForNextTick.delete(fn2);
    }
    /**
    ...
    tick(t2 = performance.now()) {
      ...
      for (const v2 of this._scheduledForNextTick) {
        this._scheduledForThisOrNextTick.add(v2);
      }
  ...
  var PointerProxy = class {
     ...
     */
    setPointer(p2) {
      this._currentPointerBox.set(p2);
    }
    /**
  ...
  var ProjectsSingleton = class {
    constructor() {
      __publicField(this, "atom", new Atom({ projects: {} }));
    }
    /**
  ...
  function interpolationTripleAtPosition(ctx, trackP, timeD) {
    return prism_default(() => {
      ...
      const driverD = prism_default.memo(
        "driver",
        () => {
          if (!track) {
            return prism_default(() => void 0);
  ...
  function _forKeyframedTrack(ctx, track, timeD) {
    return prism_default(() => {
      let stateRef = prism_default.ref("state", { started: false });
      let state2 = stateRef.current;
      const time = timeD.getValue();
      if (!state2.started || time < state2.validFrom || state2.validTo <= time) {
        stateRef.current = state2 = updateState(ctx, timeD, track);
      }
      return state2.der.getValue();
    });
  }
  var undefinedConstD = prism_default(() => void 0);
  ...
  var states = {
    beforeFirstKeyframe(kf) {
      return {
        ...
        validFrom: -Infinity,
        validTo: kf.position,
        der: prism_default(() => ({ left: kf.value, progression: 0 }))
      };
    },
    lastKeyframe(kf) {
      return {
    ...
    between(left, right, progressionD) {
      if (!left.connectedRight) {
        return {
          ...
          der: prism_default(() => ({ left: left.value, progression: 0 }))
        };
      }
  ...
  var valToAtom = (key, vals) => {
    const a2 = prism_default.memo(key, () => new Atom(vals), []);
    a2.set(vals);
    return a2;
  };
  ...
  var TheatreSheetObject = class {
    ...
    _valuesPrism() {
      return this._cache.get("_valuesPrism", () => {
        ...
        const d2 = prism_default(() => {
          return val(sheetObject.getValues().getValue());
        });
        return d2;
      });
    }
    ...
    set initialValue(val3) {
      privateAPI(this).setInitialValue(val3);
    }
  };
  ...
  function getPropConfigByPath(parentConf, path) {
    ...
    if (!isPropConfigComposite(parentConf))
      return void 0;
    const sub2 = parentConf.type === "enum" ? parentConf.cases[key] : parentConf.props[key];
    return getPropConfigByPath(sub2, rest);
  }
  ...
  var SheetObject = class {
    constructor(sheet, template, nativeObject) {
      ...
      __publicField(this, "_initialValue", new Atom({}));
      __publicField(this, "_cache", new SimpleCache());
    ...
    getValues() {
      return this._cache.get(
        ...
        () => prism_default(() => {
          ...
          const withInitialCache = prism_default.memo(
            "withInitialCache",
          ...
          const withStaticsCache = prism_default.memo(
            "withStatics",
            () => /* @__PURE__ */ new WeakMap(),
            []
          );
          ...
          let sequenced;
          {
            const pointerToSequencedValuesD = prism_default.memo(
              "seq",
              () => this.getSequencedValues(),
              []
            );
            const withSeqsCache = prism_default.memo(
              "withSeqsCache",
              () => /* @__PURE__ */ new WeakMap(),
              []
            );
    ...
    pointerToPrism(pointer2) {
      ...
      return prism_default(() => {
        const allValuesP = val(this.getValues());
        return val(pointerDeep(allValuesP, path));
      });
    }
    /**
    ...
    getSequencedValues() {
      return prism_default(() => {
        const tracksToProcessD = prism_default.memo(
          "tracksToProcess",
    ...
    get propsP() {
      return this._cache.get(
        "propsP",
        () => pointer_default({ root: this, path: [] })
      );
    }
    validateValue(pointer2, value) {
    }
    setInitialValue(val3) {
      this.validateValue(this.propsP, val3);
      this._initialValue.set(val3);
    }
  };
  ...
  function createTheatreInternalLogger(useConsole = console, _options = {}) {
    const ref2 = __spreadProps(__spreadValues({}, DEFAULTS), { includes: __spreadValues({}, DEFAULTS.includes) });
    const createConsole = {
    ...
    return {
      configureLogger(config) {
        ...
        if (config === "console") {
          ...
          ref2.loggerConsoleStyle = (_a = config.style) != null ? _a : DEFAULTS.loggerConsoleStyle;
          ref2.create = getConCreate();
      ...
      configureLogging(config) {
        var _a, _b, _c, _d, _e;
        ref2.includes.dev = (_a = config.dev) != null ? _a : DEFAULTS.includes.dev;
        ref2.includes.internal = (_b = config.internal) != null ? _b : DEFAULTS.includes.internal;
        ref2.includes.min = (_c = config.min) != null ? _c : DEFAULTS.includes.min;
        ref2.include = (_d = config.include) != null ? _d : DEFAULTS.include;
        ref2.loggingConsoleStyle = (_e = config.consoleStyle) != null ? _e : DEFAULTS.loggingConsoleStyle;
        ref2.create = getConCreate();
      },
      getLogger() {
  ...
  function createExtLogger(source2) {
    const includes = __spreadValues(__spreadValues({}, this.includes), this.include(source2));
    ...
    const _hmm = _HMM ? ext.error.bind(ext, LEVELS._hmm) : f2.bind(source2, 524 /* _HMM */);
    const _todo = _TODO ? ext.error.bind(ext, LEVELS._todo) : f2.bind(source2, 522 /* _TODO */);
    const _error = _ERROR ? ext.error.bind(ext, LEVELS._error) : f2.bind(source2, 521 /* _ERROR */);
    const errorDev = ERROR_DEV ? ext.error.bind(ext, LEVELS.errorDev) : f2.bind(source2, 529 /* ERROR_DEV */);
    const errorPublic = ERROR_PUBLIC ? ext.error.bind(ext, LEVELS.errorPublic) : f2.bind(source2, 545 /* ERROR_PUBLIC */);
    const _kapow = _KAPOW ? ext.warn.bind(ext, LEVELS._kapow) : f2.bind(source2, 268 /* _KAPOW */);
    const _warn = _WARN ? ext.warn.bind(ext, LEVELS._warn) : f2.bind(source2, 265 /* _WARN */);
    const warnDev = WARN_DEV ? ext.warn.bind(ext, LEVELS.warnDev) : f2.bind(source2, 273 /* WARN_DEV */);
    const warnPublic = WARN_PUBLIC ? ext.warn.bind(ext, LEVELS.warnPublic) : f2.bind(source2, 273 /* WARN_DEV */);
    const _debug = _DEBUG ? ext.debug.bind(ext, LEVELS._debug) : f2.bind(source2, 137 /* _DEBUG */);
    const debugDev = DEBUG_DEV ? ext.debug.bind(ext, LEVELS.debugDev) : f2.bind(source2, 145 /* DEBUG_DEV */);
    const _trace = _TRACE ? ext.trace.bind(ext, LEVELS._trace) : f2.bind(source2, 73 /* _TRACE */);
    const traceDev = TRACE_DEV ? ext.trace.bind(ext, LEVELS.traceDev) : f2.bind(source2, 81 /* TRACE_DEV */);
    const logger = {
      _hmm,
      _todo,
      _error,
  ...
  function createConsoleLoggerStyled(con, source2) {
    ...
    const styleArgs = [];
    let prefix = "";
    for (let i2 = 0; i2 < source2.names.length; i2++) {
      const { name, key } = source2.names[i2];
  ...
  function _createConsoleLogger(f2, source2, includes, con, prefix, kapowPrefix, named) {
    const _HMM = shouldLog(includes, 524 /* _HMM */);
    const _TODO = shouldLog(includes, 522 /* _TODO */);
    const _ERROR = shouldLog(includes, 521 /* _ERROR */);
    const ERROR_DEV = shouldLog(includes, 529 /* ERROR_DEV */);
    ...
    const _TRACE = shouldLog(includes, 73 /* _TRACE */);
    const TRACE_DEV = shouldLog(includes, 81 /* TRACE_DEV */);
    const _hmm = _HMM ? con.error.bind(con, ...prefix) : f2.bind(source2, 524 /* _HMM */);
    const _todo = _TODO ? con.error.bind(con, ...prefix) : f2.bind(source2, 522 /* _TODO */);
    const _error = _ERROR ? con.error.bind(con, ...prefix) : f2.bind(source2, 521 /* _ERROR */);
    const errorDev = ERROR_DEV ? con.error.bind(con, ...prefix) : f2.bind(source2, 529 /* ERROR_DEV */);
    const errorPublic = ERROR_PUBLIC ? con.error.bind(con, ...prefix) : f2.bind(source2, 545 /* ERROR_PUBLIC */);
    const _kapow = _KAPOW ? con.warn.bind(con, ...kapowPrefix) : f2.bind(source2, 268 /* _KAPOW */);
    const _warn = _WARN ? con.warn.bind(con, ...prefix) : f2.bind(source2, 265 /* _WARN */);
    const warnDev = WARN_DEV ? con.warn.bind(con, ...prefix) : f2.bind(source2, 273 /* WARN_DEV */);
    const warnPublic = WARN_PUBLIC ? con.warn.bind(con, ...prefix) : f2.bind(source2, 273 /* WARN_DEV */);
    const _debug = _DEBUG ? con.info.bind(con, ...prefix) : f2.bind(source2, 137 /* _DEBUG */);
    const debugDev = DEBUG_DEV ? con.info.bind(con, ...prefix) : f2.bind(source2, 145 /* DEBUG_DEV */);
    const _trace = _TRACE ? con.debug.bind(con, ...prefix) : f2.bind(source2, 73 /* _TRACE */);
    const traceDev = TRACE_DEV ? con.debug.bind(con, ...prefix) : f2.bind(source2, 81 /* TRACE_DEV */);
    const logger = {
      _hmm,
      _todo,
      _error,
  ...
  var SheetObjectTemplate = class {
    ...
    getStaticValues() {
      return this._cache.get(
        ...
        () => prism_default(() => {
          var _a;
          const json = (_a = val(this.pointerToStaticOverrides)) != null ? _a : {};
          const config = val(this.configPointer);
  ...
  var DefaultPlaybackController = class {
    constructor() {
      ...
      __publicField(this, "_state", new Atom({
        position: 0,
        playing: false
      }));
  ...
  var Sequence = class {
    constructor(_project, _sheet, _lengthD, _subUnitsPerUnitD, playbackController) {
      ...
      __publicField(this, "pointer", pointer_default({ root: this, path: [] }));
      __publicField(this, "$$isPointerToPrismProvider", true);
      __publicField(this, "_logger");
    ...
    getKeyframesOfSimpleProp(prop) {
      const { path, root: root2 } = getPointerParts(prop);
  ...
  function sanitizeCompoundProps(props) {
    ...
    for (const key of Object.keys(props)) {
      ...
      const val3 = props[key];
      if (isLonghandPropType(val3)) {
        sanitizedProps[key] = val3;
      } else {
        sanitizedProps[key] = toLonghandProp(val3);
      }
    }
    ...
    if (false) {
      ...
      if (Object.prototype.hasOwnProperty.call(opts, "label")) {
        ...
        if (typeof label !== "string") {
          throw new Error(
            "opts.label in ".concat(fnCallSignature, " should be a string. ").concat(userReadableTypeOfValue2(
              label
            ), " given.")
          );
        }
  ...
  var _ensureFile = (val3) => {
    if (!val3)
      return void 0;
    let valid = true;
    if (typeof val3.id !== "string" && ![null, void 0].includes(val3.id)) {
      valid = false;
    }
    if (val3.type !== "file")
      valid = false;
  ...
  var number = (defaultValue, opts = {}) => {
    ...
    if (false) {
      ...
      if (typeof defaultValue !== "number" || !isFinite(defaultValue)) {
        throw new Error(
          "Argument defaultValue in t.number(defaultValue) must be a number. ".concat(userReadableTypeOfValue2(
            defaultValue
          ), " given.")
        );
      }
      if (typeof opts === "object" && opts !== null) {
        if (Object.prototype.hasOwnProperty.call(opts, "range")) {
          if (!Array.isArray(opts.range)) {
            throw new Error(
              "opts.range in t.number(defaultValue, opts) must be a tuple of two numbers. ".concat(userReadableTypeOfValue2(
                opts.range
              ), " given.")
            );
          }
        ...
        if (Object.prototype.hasOwnProperty.call(opts, "nudgeMultiplier")) {
          if (typeof opts.nudgeMultiplier !== "number" || !isFinite(opts.nudgeMultiplier)) {
            throw new Error(
              "opts.nudgeMultiplier in t.number(defaultValue, opts) must be a finite number. ".concat(userReadableTypeOfValue2(
        ...
        if (Object.prototype.hasOwnProperty.call(opts, "nudgeFn")) {
          if (typeof opts.nudgeFn !== "function") {
            throw new Error(
              "opts.nudgeFn in t.number(defaultValue, opts) must be a function. ".concat(userReadableTypeOfValue2(
                opts.nudgeFn
              ), " given.")
            );
          }
  ...
  var _sanitizeRgba = (val3) => {
    if (!val3)
      return void 0;
    let valid = true;
    for (const c2 of ["r", "g", "b", "a"]) {
      if (!Object.prototype.hasOwnProperty.call(val3, c2) || typeof val3[c2] !== "number") {
        valid = false;
      }
    }
    if (!valid)
  ...
  var boolean = (defaultValue, opts = {}) => {
    ...
    if (false) {
      ...
      if (typeof defaultValue !== "boolean") {
        throw new Error(
          "defaultValue in t.boolean(defaultValue) must be a boolean. ".concat(userReadableTypeOfValue2(
            defaultValue
          ), " given.")
        );
      }
  ...
  var _ensureBoolean = (val3) => {
    return typeof val3 === "boolean" ? val3 : void 0;
  };
  ...
  var string = (defaultValue, opts = {}) => {
    ...
    if (false) {
      ...
      if (typeof defaultValue !== "string") {
        throw new Error(
          "defaultValue in t.string(defaultValue) must be a string. ".concat(userReadableTypeOfValue2(
            defaultValue
          ), " given.")
        );
      }
  ...
  var Sheet = class {
    constructor(template, instanceId) {
      ...
      __publicField(this, "_objects", new Atom({}));
    ...
    deleteObject(objectKey) {
      this._objects.reduce((state2) => {
        const newState = __spreadValues({}, state2);
    ...
    getSequence() {
      if (!this._sequence) {
        ...
        const subUnitsPerUnitD = prism_default(() => {
          const unsanitized = val(
  ...
  var SheetTemplate = class {
    constructor(project, sheetId) {
      ...
      __publicField(this, "address");
      __publicField(this, "_instances", new Atom({}));
      __publicField(this, "instancesP", this._instances.pointer);
      __publicField(this, "_objectTemplates", new Atom({}));
  ...
  async function initialiseProjectState(studio, project, onDiskState) {
    ...
    studio.transaction(({ drafts }) => {
      ...
      function useOnDiskState(state2) {
        drafts.ephemeral.coreByProject[projectId].loadingState = {
          type: "loaded"
        };
        drafts.historic.coreByProject[projectId] = state2;
      }
  ...
  var Project = class {
    constructor(id, config = {}, publicApi) {
      ...
      __publicField(this, "_sheetTemplates", new Atom({}));
      ...
      const onDiskStateAtom = new Atom({
        ahistoric: {
          ahistoricStuff: ""
        },
  ...
  function getProject(id, config = {}) {
    ...
    if (existingProject) {
      if (false) {
        if (!deepEqual2(config, existingProject.config)) {
          throw new Error(
  ...
  function onChange(pointer2, callback, rafDriver) {
    const ticker = rafDriver ? privateAPI(rafDriver).ticker : getCoreTicker();
    if (isPointer(pointer2)) {
      const pr = pointerToPrism(pointer2);
      return pr.onChange(ticker, callback, true);
    } else if (isPrism(pointer2)) {
      return pointer2.onChange(ticker, callback, true);
    } else {
      throw new Error(
  ...
  function val2(pointer2) {
    if (isPointer(pointer2)) {
      return pointerToPrism(pointer2).getValue();
    } else {
