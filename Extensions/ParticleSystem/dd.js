!function (a) {
    var b = function (b) {
        if (window.$ === a) throw "jQuery must be included to use cloudkid.UpdateChecker"; b = b || 2; var c = localStorage.getItem("lastUpdateCheck") || 0; if (!(Date.now() - c <= 1e3 * b * 3600)) {
            this.repository = null, this.currentTag = null, this._destroyed = !1; var d = "package.json", e = this; $.getJSON(d, function (a) {
                if (!e._destroyed && a.repository && a.repository.url.search(/github\.com/) !== -1) {
                    e.currentTag = a.version, e.repository = a.repository.url; var b = a.repository.url.replace("http:", "https:").replace("github.com", "api.github.com/repos") + "/releases"; $.getJSON(b, e.onTagsLoaded)
                }
            }), this.onTagsLoaded = this.onTagsLoaded.bind(this)
        }
    }, c = b.prototype; c.onTagsLoaded = function (a) {
        if (!this._destroyed && (localStorage.setItem("lastUpdateCheck", Date.now()), a && Array.isArray(a) && 0 !== a.length)) {
            var b, c, d, e = require("semver"), f = a.length; for (b = 0; b < f; b++)if (c = a[b], d = c.tag_name, !c.prerelease && e.valid(d) && e.gt(d, this.currentTag)) {
    ...
    var b, c = function () {
        if (b = this, window.$ === a) throw "jQuery must be included to use cloudkid.Browser"; this.file = $('<input type="file" />').css("visibility", "hidden").change(function (a) {
            var c = $(this); c.removeAttr("accept"); var d = c.val(), e = b._fileCallback; b._fileCallback = null, c.val(""), e(d)
        }), this.saveAs = $('<input type="file" nwsaveas />').css("visibility", "hidden").change(function (a) {
            var c = $(this); c.attr("nwsaveas", ""); var d = c.val(), e = b._fileCallback; b._fileCallback = null, c.val(""), e(d)
        }); var c = "webkitdirectory"; this.folder = $('<input type="file" ' + c + " />").css("visibility", "hidden").change(function (a) {
            var c = $(this), d = c.val(), e = b._folderCallback; b._folderCallback = null, c.val(""), e(d)
        }), $("body").append(this.file, this.folder, this.saveAs), this._fileCallback = null, this._folderCallback = null
    }, d = c.prototype = {
    }; c.init = function () {
        if (b) throw "Only once instance file created at once"; return new c
    }, Object.defineProperty(c, "instance", {
        get: function () {
            return b
        }
    }), c.folder = function (a, c) {
        if (!b) throw "Call cloudkid.Browser.init() first"; b.folder.removeAttr("nwworkingdir"), b._folderCallback = a, b.folder.trigger("click")
    }, c.file = function (a, c, d) {
        if (!b) throw "Call cloudkid.Browser.init() first"; b.file.removeAttr("accept"), c && b.file.attr("accept", c), b._fileCallback = a, b.file.trigger("click")
    }, c.saveAs = function (a, c, d) {
        if (!b) throw "Call cloudkid.Browser.init() first"; b.saveAs.attr("nwsaveas", c || ""), b._fileCallback = a, b.saveAs.trigger("click")
    }, c.destroy = function () {
        b && b.destroy()
    }, d.destroy = function () {
    ...
    a.cloudkid = a.cloudkid || {
        ...
        this.updater = null, this.browser = null, this.gui = null, this.main = null, this.resizable = !0, this.packageData = null; a.onerror = this._handleErrors.bind(this)
    }, d = c.prototype; d._handleErrors = function (a) {
        alert(a)
    }, d._onClose = function () {
        var a = this.main, b = this.gui, c = {
            x: a.x, y: a.y
    ...
    var EventDispatcher = include("springroll.EventDispatcher"), EditorInterface = function (a) {
        EventDispatcher.call(this), this.spawnTypes = a; for (var b = ["alphaStart", "alphaEnd", "scaleStart", "scaleEnd", "minimumScaleMultiplier", "colorStart", "colorEnd", "speedStart", "speedEnd", "minimumSpeedMultiplier", "accelX", "accelY", "maxSpeed", "startRotationMin", "startRotationMax", "partNoRotation", "rotationSpeedMin", "rotationSpeedMax", "lifeMin", "lifeMax", "blendMode", "customEase", "subclass", "extraData", "emitFrequency", "emitLifetime", "emitMaxParticles", "emitSpawnPosX", "emitSpawnPosY", "emitAddAtBack", "emitSpawnType", "emitRectX", "emitRectY", "emitRectW", "emitRectH", "emitCircleX", "emitCircleY", "emitCircleR", "emitRingX", "emitRingY", "emitRingR", "emitRingMinR", "emitParticlesPerWave", "emitParticleSpacing", "emitAngleStart", "defaultConfigSelector", "defaultImageSelector", "configUpload", "configPaste", "imageUpload", "imageDialog", "imageList", "refresh", "loadConfig", "downloadConfig", "configDialog", "configConfirm", "imageConfirm", "addImage", "stageColor", "content", "renderer"], c = 0; c < b.length; c++)this[b[c]] = $("#" + b[c]); this.downloadConfig.click(this.download.bind(this)), this.init()
    }, p = extend(EditorInterface, EventDispatcher); p.changed = function () {
        this.trigger("change")
    }, p.init = function () {
        var a = this, b = (springroll.Application.instance, this.changed.bind(this)); $("[data-toggle='tooltip']").tooltip({
            container: "body", animation: !1
        }), this.alphaStart.slider().data("slider").on("slide", b), this.alphaEnd.slider().data("slider").on("slide", b), $(".spinner").each(function () {
            $(this).TouchSpin({
                verticalbuttons: !0, min: void 0 !== $(this).attr("data-min") ? parseFloat($(this).attr("data-min")) : -1e6, max: void 0 !== $(this).attr("data-max") ? parseFloat($(this).attr("data-max")) : 1e6, step: parseFloat($(this).attr("data-step")) || 1, decimals: void 0 !== $(this).attr("data-decimals") ? parseFloat($(this).attr("data-decimals")) : 1, forcestepdivisibility: "none"
            })
            ...
            $(this).minicolors({
                control: "hue", defaultValue: "", inline: !1, letterCase: "lowercase", position: "top right", change: function (a, c) {
                    a && b()
                }, theme: "bootstrap"
            })
        ...
        }), this.blendMode.on("change", b), this.customEase.on("input", b), this.subclass.on("change", b), this.extraData.on("input", b); var c = this.spawnTypes; this.emitSpawnType.change(function (b) {
        ...
        } var frequency = this.emitFrequency.val(); output.frequency = parseFloat(frequency) > 0 ? parseFloat(frequency) : .5, output.emitterLifetime = parseFloat(this.emitLifetime.val()) || -1, output.maxParticles = parseInt(this.emitMaxParticles.val()) || 1e3, output.pos = {
            x: parseFloat(this.emitSpawnPosX.val() || 0), y: parseFloat(this.emitSpawnPosY.val() || 0)
        }, output.addAtBack = this.emitAddAtBack.prop("checked"); var spawnType = output.spawnType = this.emitSpawnType.val(); return "rect" == spawnType ? output.spawnRect = {
            x: parseFloat(this.emitRectX.val()) || 0, y: parseFloat(this.emitRectY.val()) || 0, w: parseFloat(this.emitRectW.val()) || 0, h: parseFloat(this.emitRectH.val()) || 0
        } : "circle" == spawnType ? output.spawnCircle = {
            x: parseFloat(this.emitCircleX.val()) || 0, y: parseFloat(this.emitCircleY.val()) || 0, r: parseFloat(this.emitCircleR.val()) || 0
        } : "ring" == spawnType ? output.spawnCircle = {
            x: parseFloat(this.emitRingX.val()) || 0, y: parseFloat(this.emitRingY.val()) || 0, r: parseFloat(this.emitRingR.val()) || 0, minR: parseFloat(this.emitRingMinR.val()) || 0
        ...
        var a = JSON.stringify(this.get(), null, "\t"), b = "data:application/json;charset=utf-8", c = !1; try {
            c = !!new Blob
        } catch (d) {
    ...
    function init() {
        jqImageDiv = $(".particleImage"), jqImageDiv.remove(), particleCountDiv = document.getElementById("particleCount"); var a = parseInt(SavedData.read("stageColor") || "999999", 16); backgroundSprite = new Sprite(Texture.fromImage("assets/images/bg.png")), backgroundSprite.tint = a, emitterContainer = new Container; var b = {
            clearView: !0, backgroundColor: a
    ...
    }, p = extend(Editor, Application), stage, interaction, backgroundSprite, emitter, emitterContainer, emitterEnableTimer = 0, particleDefaults = {
    }, particleDefaultImages = {
    }, particleDefaultImageUrls = {
    }, jqImageDiv = null, particleCountDiv = null; p.onResize = function (a, b) {
        ...
        var b, c, d, e, f = this.config.images; for (e = 0; e < this.config.emitters.length; e++) {
            b = this.config.emitters[e], d = b.id, particleDefaultImageUrls[d] = [], particleDefaultImages[d] = []; for (var g = 0; g < b.images.length; g++)c = b.images[g], particleDefaultImageUrls[d].push(f[c]), particleDefaultImages[d].push(this.getCache(c))
        } emitter = new Emitter(emitterContainer), window.emitter = emitter; var h, i = window.location.hash.replace("#", ""); try {
            h = SavedData.read("customConfig"), f = SavedData.read("customImages")
        } catch (j) {
        } if (i) this.loadDefault(i); else if (h && f) for (this.loadSettings(getTexturesFromUrls(f), h), this.setConfig(h), e = 0; e < f.length; ++e)this.addImage(f[e]); else this.loadDefault(this.config["default"]); this.on({
            resize: this._centerEmitter.bind(this), update: this.update.bind(this)
        })
        ...
        for (var b = [], c = 0; c < a.length; ++c)b[c] = Texture.fromImage(a[c]); return b
    }; p.loadConfig = function (event) {
        var ui = this.ui, type, value, success = !1; if ("-Default Emitters-" != ui.defaultConfigSelector.val() ? type = "default" : ui.configPaste.val() ? type = "paste" : ui.configUpload[0].files.length > 0 && (type = "upload"), "default" == type) value = ui.defaultConfigSelector.val(), "-Default Emitters-" != value && (success = !0, this.loadDefault(value)); else if ("paste" == type) {
            value = ui.configPaste.val(); try {
            ...
            }, file = files[0], reader = new FileReader; reader.onloadend = onloadend.bind(this, reader), reader.readAsText(file), success = !0
        } success && ui.configDialog.modal("hide")
    }, p.loadImage = function (a) {
        var b, c, d = this.ui, e = !1; if ("-Default Images-" != d.defaultImageSelector.val() ? b = "select" : d.imageUpload[0].files.length > 0 && (b = "upload"), "select" == b) c = d.defaultImageSelector.val(), "-Default Images-" != c && (e = !0, this.addImage(c), this.loadFromUI()); else if ("upload" == b) {
            for (var f = function (a) {
                this.addImage(a.result), this.loadFromUI()
            }, g = d.imageUpload[0].files, h = 0; h < g.length; h++) {
                var i = g[h], j = new FileReader; j.onloadend = f.bind(this, j), j.readAsDataURL(i)
            } e = !0
        } e && d.imageDialog.modal("hide")
    }, p.addImage = function (a) {
        var b = jqImageDiv.clone(); b.children("img").prop("src", a), this.ui.imageList.append(b), b.children(".remove").click(removeImage.bind(this)), b.children(".download").click(downloadImage)
    }; var downloadImage = function (a) {
        var b = $(a.delegateTarget).siblings("img").prop("src"); window.open(b)
    }, removeImage = function (a) {
        $(a.delegateTarget).parent().remove(), this.loadFromUI()
    }; p.loadFromUI = function () {
        window.location.hash = ""; var a = this.ui.get(), b = this.getTexturesFromImageList(), c = this.getParticleClass(); SavedData.write("customConfig", a), this.loadSettings(b, a, c)
    }, p.getParticleClass = function () {
        var a = this.ui.getParticleClass(); switch (a) {
            case "path": return PIXI.particles.PathParticle; case "anim": return PIXI.particles.AnimatedParticle; default: return PIXI.particles.Particle
        }
    }, p.getTexturesFromImageList = function () {
        var a = [], b = this.ui.imageList.find("img"); if (0 === b.length) return null; return b.each(function () {
            a.push(this.src)
        ...
        if (emitter) {
            var b = a.data; emitter.updateOwnerPos(b.global.x, b.global.y)
        }
    }, namespace("pixiparticles").Editor = Editor
}(), function () {
    var a = include("cloudkid.NodeWebkitApp"), b = include("pixiparticles.Editor"), c = (include("pixiparticles.Menu", !1), include("cloudkid.Browser"), function () {
        a.apply(this), this.editor = new b({
            framerate: "framerate", fps: 60, raf: !0, debug: !1, resizeElement: "content", uniformResize: !1, responsive: !0
